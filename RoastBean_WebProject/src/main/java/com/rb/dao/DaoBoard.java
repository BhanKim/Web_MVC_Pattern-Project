...
//		replyShape(bGroup, bStep);
	...
	// List view
	public ArrayList<DtoBoard> myboard(String id, int curPage) {
	       ...
	       ResultSet resultSet = null;
	        ...
	        try {
	           con = dataSource.getConnection();
	          String query = "select count(*) as total from community where community_deletedate is null and community_name = ? ";
	          //총 게시물의 수를 구해주고 이걸 페이지마다 나눠서 보여주는 역할을 함
	          pstmt = con.prepareStatement(query);
	       ...
	       try {
	          ...
	          String query = "select * from (select row_number() over(order by community_group, community_indent desc)"
	                + "as rownum, community_id, u.user_nick, community_title, community_content, community_initdate, community_updatedate, community_deletedate, community_hit, community_group, community_step, community_indent, community_cnt "
	                + "from community as c, `user` as u  where community_deletedate is null and u.user_id != 'admin' and u.user_id = c.community_name and c.community_name = ? order by community_group desc, community_indent)A where rownum <= ? and rownum >= ? " ;
	          pstmt = con.prepareStatement(query);
	          pstmt.setString(1, id);
	          pstmt.setInt(2, nEnd);//끝나는 범위를 먼저
	          pstmt.setInt(3, nStart);
	          resultSet = pstmt.executeQuery();
	   ...
	   // myboard 게시글 페이징
		public PageInfo myboardpage(String id, int curPage) {
