	...
	public ArrayList<DtoBoard> list(int curPage) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				DtoBoard dto = new DtoBoard(rownum, community_id, community_name, community_title, community_content,
						community_initdate, community_updatedate, community_deletedate, community_hit, community_group,
	...
	public ArrayList<DtoBoard> noticeList() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				DtoBoard dto = new DtoBoard(community_id, community_name, community_title, community_content,
						community_initdate, community_updatedate, community_deletedate, community_hit, community_group,
						community_step, community_indent, community_cnt);
				dtos.add(dto);
			}
		...
		try {
			...
			if (b_opt.equals("1")) {
				String query = "select * from community " + " where community_title like '%" + keyword
						+ "%' or community_content like '%" + keyword + "%' " + " order by community_initdate desc";
				pstmt = con.prepareStatement(query);
...
//		 		pstmt.setInt(4, nStart);
				...
				while (resultSet.next()) {
					...
					DtoBoard dto = new DtoBoard(community_id, community_name, community_title, community_content,
							community_initdate, community_hit, community_group, community_step, community_indent,
							community_cnt);
					dtos.add(dto);
				}
